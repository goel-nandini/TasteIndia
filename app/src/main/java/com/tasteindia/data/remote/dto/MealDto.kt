package com.tasteindia.data.remote.dto

import kotlinx.serialization.SerialName
import kotlinx.serialization.Serializable

/**
 * The `lookup.php?i={id}` record, mirroring TheMealDB's V1 response exactly.
 *
 * Every field is nullable because the provider genuinely returns nulls — and,
 * more awkwardly, whitespace-only strings — for unused ingredient slots, missing
 * videos and absent tags. The twenty ingredient/measure pairs are declared
 * explicitly rather than parsed dynamically: it is more verbose, but it is
 * type-checked, needs no custom serializer, and makes the normalisation in
 * [ingredientPairs] obvious to read and to test.
 *
 * This type never leaves the data layer.
 */
@Serializable
data class MealDto(
    ...
    /**
     * The twenty slots as ordered pairs, before any cleaning. Kept separate from
     * the cleaning step so the mapper can be read as "collect, then normalise".
     */
    val ingredientPairs: List<Pair<String?, String?>>
        get() = listOf(
...
/** Envelope for `lookup.php` and `search.php`. `meals` is null when nothing matched. */
@Serializable
data class MealsResponse(
    @SerialName("meals") val meals: List<MealDto>? = null,
)

/** The reduced record returned by every `filter.php` query. */
@Serializable
...
@Serializable
data class MealSummariesResponse(
    @SerialName("meals") val meals: List<MealSummaryDto>? = null,
)

/** `list.php?c=list` — used to validate the categories we derive from the data. */
@Serializable
...
data class CategoryListResponse(
    @SerialName("meals") val meals: List<CategoryNameDto>? = null,
)
